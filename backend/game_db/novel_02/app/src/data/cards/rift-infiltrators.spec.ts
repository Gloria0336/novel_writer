...
describe("T_s_35–T_s_40 卡定義完整性", () => {
  it("RIFT_INFILTRATOR_CARDS 共 6 張，ID T_s_35–T_s_40", () => {
    expect(RIFT_INFILTRATOR_CARDS).toHaveLength(6);
    expect(RIFT_INFILTRATOR_CARDS.map((c) => c.id)).toEqual(["T_s_35", "T_s_36", "T_s_37", "T_s_38", "T_s_39", "T_s_40"]);
  });

  it("getCard 可取得 T_s_35–T_s_40", () => {
    for (const id of ["T_s_35", "T_s_36", "T_s_37", "T_s_38", "T_s_39", "T_s_40"]) {
      const c = getCard(id);
      expect(c.id).toBe(id);
  ...
  it("T_s_35–T_s_40 全為 troop card 且有 flavor", () => {
...
describe("T_s_35 黑暗史萊姆（1 費 8/3/1）— 謝幕曲：對隨機玩家單位 2 腐蝕傷害", () => {
  it("基準數值", () => {
    const c = getCard("T_s_35");
    if (c.type !== "troop") throw new Error("not troop");
  ...
  it("謝幕曲：對玩家單位 2 傷害（無視 DEF）", () => {
    const s = mkState();
    const m01 = mkInfiltrator("T_s_35");
    s.enemy.troopSlots[0] = m01;
    // 玩家方有 1 兵力 + 英雄
    const c = getCard("T_c_03");
    if (c.type !== "troop") throw new Error("not troop");
    s.player.troopSlots[0] = {
      instanceId: "pt1", cardId: "T_c_03",
      hp: c.hp, maxHp: c.hp, atk: c.atk, def: c.def,
      keywords: new Set(c.keywords),
      hasAttackedThisTurn: false, summonedThisTurn: false, frozenTurns: 0, buffs: [],
    }; // T_c_03 守護兵 10/2/5
    const m01Card = getCard("T_s_35");
    if (m01Card.type !== "troop" || !m01Card.onDestroy) throw new Error("expect onDestroy");
    executeEffects(m01Card.onDestroy, {
      state: s, ctx, sourceSide: "enemy", sourceKind: "troop_destroy", sourceInstanceId: m01.instanceId, sourceCardId: "T_s_35",
    });
    // 隨機 1 名玩家方單位（target.ts random 返回前 1 個，即英雄）
    // 玩家英雄 DEF 5，但 ignoreDef → -2 HP
    // 或者命中兵力 T_c_03 → ignoreDef 直接扣 2 HP
...
describe("T_s_36 觸手蠕行體（2 費 12/5/2）", () => {
  it("基準數值（簡化版：純數值，passive 留 Stage 3）", () => {
    const c = getCard("T_s_36");
    if (c.type !== "troop") throw new Error("not troop");
...
describe("T_s_37 古魔咆哮者（3 費 15/8/2）— 入場曲：對玩家英雄 3 純黑暗傷害（無視 DEF）", () => {
  it("基準數值與〔突進〕關鍵字", () => {
    const c = getCard("T_s_37");
    if (c.type !== "troop") throw new Error("not troop");
  ...
  it("入場曲：玩家英雄 -3 HP（無視 DEF 5）", () => {
    ...
    executeEffects(c.onPlay, {
      state: s, ctx, sourceSide: "enemy", sourceKind: "troop_play", sourceCardId: "T_s_37",
    });
    expect(s.player.hero.hp).toBe(77); // 80 - 3（無視 DEF）
  });
});

describe("T_s_38 夢魔影刺（4 費 10/9/1）— 〔疾走〕", () => {
  it("基準數值與〔疾走〕關鍵字", () => {
    const c = getCard("T_s_38");
    if (c.type !== "troop") throw new Error("not troop");
...
describe("T_s_39 冰魔施法者（5 費 14/6/3）— 入場曲：對所有玩家兵力 3 傷 + ATK -2", () => {
  it("基準數值", () => {
    const c = getCard("T_s_39");
    if (c.type !== "troop") throw new Error("not troop");
  ...
  it("入場曲：所有玩家兵力 -3 HP + ATK -2 / 1 回合", () => {
    const s = mkState();
    const t1 = getCard("T_c_02"); // 12/5/2
    const t2 = getCard("T_c_03"); // 10/2/5
    if (t1.type !== "troop" || t2.type !== "troop") throw new Error();
    s.player.troopSlots[0] = {
      instanceId: "pt1", cardId: "T_c_02",
    ...
    if (c.type !== "troop" || !c.onPlay) throw new Error("expect onPlay");
    executeEffects(c.onPlay, {
      state: s, ctx, sourceSide: "enemy", sourceKind: "troop_play", sourceCardId: "T_s_39",
    });
    // T_c_02 (DEF 2): 3 - 2 = 1 → 12 - 1 = 11
    // T_c_03 (DEF 5): 3 - 5 仍有最低 1 點傷害 → 10 - 1 = 9
...
describe("T_s_40 巨魔碎牆者（6 費 22/9/7）— 純高坦 + 〔守護〕", () => {
  it("基準數值與〔守護〕關鍵字", () => {
    const c = getCard("T_s_40");
    if (c.type !== "troop") throw new Error("not troop");
